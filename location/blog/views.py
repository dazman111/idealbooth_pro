...
def article_detail(request, article_id):
    ...
    if request.method == 'POST' and request.user.is_authenticated:
        ...
        if form.is_valid():
            ...
            # Pas de redirect on renvoie la page directement (200 OK)
            form = CommentForm()  # réinitialiser le formulaire

            return render(
                request,
                'blog/article_detail.html',
                {'article': article, 'comments': article.comments.all(), 'form': form},
                status=200
            )
...
def edit_comment(request, comment_id):
    ...
    # Sécurité : l'utilisateur doit être le propriétaire
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # ❌ PLUS DE REDIRECT → 302 supprimé
            # ✔️ On recharge la même page en 200
            return render(
                request,
